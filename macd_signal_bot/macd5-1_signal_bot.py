...
TIMEFRAMES = ["1m","3m", "5m","15m","30m"]  # 5个周期同时满足条件时告警
...
# 定时轮询配置
POLL_INTERVAL = 60  # 轮询间隔（秒），因为监控1分钟周期，改为60秒

# 状态文件路径
STATE_FILE = os.path.join(os.path.dirname(__file__), ".macd_state_5-1.json")

# 企业微信配置
SEND_WECHAT = True
WECHAT_WEBHOOK_URL = os.getenv("MACD_WECHAT_WEBHOOK_URL", "https://qyapi.weixin.qq.com/cgi-bin/webhook/send?key=7797beb8-534b-48ed-ab64-470202e333cc")
...
def format_macd_message(symbol: str, result: Dict) -> str:
    ...
    message += f"\n✅ {len(TIMEFRAMES)}个周期同时满足{direction_text}条件！"
...
def main():
    ...
    print(f"💡 因监控1分钟周期，轮询间隔设为60秒")
    print(f"\n开始监控... (按 Ctrl+C 停止)\n")
    print(f"⚠️  注意：只有当{len(TIMEFRAMES)}个周期同时满足条件时才会发送告警\n")
