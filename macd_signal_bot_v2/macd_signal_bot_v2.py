...
核心逻辑：
1. 在3分钟K线收盘后检测
2. 1h使用当前实时K线判断反转（及时捕捉反转信号）
3. 15m/5m/3m使用已收盘K线检查共振（数据稳定可靠）
4. 方向判断：只看Δhist = hist_t - hist_(t-1)
   - Δhist > 0 → 向上
   - Δhist < 0 → 向下
5. 10分钟内不重复发送同方向信号
...
def detect_1h_reversal(df_1h: pd.DataFrame, symbol: str = "", debug: bool = False) -> Optional[str]:
    ...
    返回:
        ...
        None: 无反转
    """
    if len(df_1h) < 3:  # 需要3根K线：倒数1,2,3
        return None
    
    # 获取当前方向（使用实时K线，因为1h级别需要及时反应）
    curr_dir = detect_direction(df_1h, "1h", symbol, debug, use_closed_candles=False)
    if curr_dir is None:
        return None
    
    # 计算前一根的Δhist（使用倒数第2和第3根，都是已收盘）
    prev_hist = df_1h.iloc[-2]["macd_hist"]
    prev2_hist = df_1h.iloc[-3]["macd_hist"]
    prev2_timestamp = df_1h.iloc[-3]["timestamp"]
...
def check_symbol_signal(exchange: ccxt.binance, symbol: str, debug: bool = False) -> Optional[Dict]:
    ...
    try:
        ...
        # 3. 记录1h数据（使用当前实时K线）
        current_hist = df_1h.iloc[-1]["macd_hist"]
        prev_hist = df_1h.iloc[-2]["macd_hist"]
        delta = current_hist - prev_hist
...
def is_already_sent(state: Dict[str, int], symbol: str, direction: str, timestamp: int) -> bool:
    """检查该信号是否已经发送过（10分钟内不重复发送）"""
    key = f"{symbol}_{direction}"
    last_timestamp = state.get(key, 0)
    # 如果10分钟内发送过，不重复发送
    if last_timestamp > 0 and (timestamp - last_timestamp) < 600:
        return True
    return False
...
def process_symbol(exchange: ccxt.binance, symbol: str, state: Dict[str, int], debug: bool = False) -> bool:
    ...
    try:
        ...
        if is_already_sent(state, symbol, direction, current_timestamp):
            if debug:
                print(f"⚠️  {symbol} {direction} 信号在10分钟内已发送过，跳过")
            return False
