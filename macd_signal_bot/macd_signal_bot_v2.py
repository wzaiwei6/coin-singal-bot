...
def detect_direction(df: pd.DataFrame, timeframe: str = "", symbol: str = "", debug: bool = False) -> Optional[str]:
    """
    检测单个周期的柱方向（基于 Δhist）
    ...
    if debug and timeframe and symbol:
        ...
        print(f"  [{timeframe}] 当前K线: {current_time_str}, hist={current_hist:.8f}")
        print(f"  [{timeframe}] 前一K线: {prev_time_str}, hist={prev_hist:.8f}")
...
def detect_1h_reversal(df_1h: pd.DataFrame, symbol: str = "", debug: bool = False) -> Optional[str]:
    ...
    # 获取当前方向
    curr_dir = detect_direction(df_1h, "1h", symbol, debug)
    if curr_dir is None:
        return None
    ...
    if debug and symbol:
        ...
        print(f"  [1h] 前二K线: {prev2_time_str}, hist={prev2_hist:.8f}")
...
def check_resonance(exchange: ccxt.binance, symbol: str, reversal_direction: str, debug: bool = False) -> Optional[Dict]:
    ...
    for timeframe in RESONANCE_TIMEFRAMES:
        try:
            ...
            # 检测方向
            direction = detect_direction(df, timeframe, symbol, debug)
